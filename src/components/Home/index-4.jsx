...
import HofTableWrapper from "../TableBox/index-hof";
...
export default class Home4 extends React.Component {
  ...
  async loadHofData(){
    ...
    let hData = await window.$myContract.methods.getHallOfFame().call();
    if(hData==="" || hData===null)return;
    let row = hData.split("@");
    let data = [];
    for(let i=0; i<row.length; i++){
        ...
        let record = {
            "player": col[0],
            "tot_revenue": window.$web3.utils.fromWei(col[1]),
            "tot_profit": window.$web3.utils.fromWei(col[2])
        }
        data.push(record);
    }
    this.setState({hofData:data});
  }
  async componentDidMount() {
      await this.loadHofData();
      ...
      Home4.gst1 = parseInt(dt1.format('HH'));
      Home4.gst2 = parseInt(dt2.format('HH'));
      this.checkForWallet();
      this.start();
  ...
  start() {
      ...
      if(Home4.gst >= 0) this.startTimer();
  ...
  async selectStartTime() {
      ...
      Home4.gst = -1;
      if(blockTimeStamp >= st1 && blockTimeStamp <= end1) {Home4.gst = Home4.gst1; Home4.tn=0;}
      if(blockTimeStamp >= st2 && blockTimeStamp <= end2) {Home4.gst = Home4.gst2; Home4.tn=1;}
  }
  async getTimeInfo(){
      let blockTimeStamp = await window.$myContract.methods.getTimestamp().call();
      let st1 = await window.$myContract.methods.getFirstStartTime().call();
  ...
  render(){
    return (
      <>
        ...
        <HofTableWrapper title="Hall of Fame" data={this.state.hofData}/>
        <Banner>
          <img src={Banner_300x250} alt="banner_300x250" />
        </Banner>
        <Footer />
